...
import time
import utilidades.plugins.accsnuker
...
global useragent
def useragent():
    file = open('datos/userlogin.txt')
    userlogin = (random.choice(list(file)))
    userlogin2 = []
    userlogin2.append(userlogin)
    userlogin1 = []

try:
    with open('datos/logueos.json') as f:
        config = json.load(f)
except:
    with open('datos/logueos.json') as f:
        print('Logueando en NOVASUN')
        login = input('[?] Usuario:')
        json.dump({'Login':login}, f, indent=4)
    input(f'[✓] Logueado correctamente como: [{lr}{login}]\nPor favor presione ENTER:')
    pass
with open('datos/logueos.json') as f:
    config = json.load(f)
...
try:
    with open('datos/lang.json') as f:
        config = json.load(f)
except:
    with open('datos/lang.json') as f:
        print('Select Lang')
    ...
    input(f'[✓] Language selected: {y}{lang}')
    pass
with open('datos/lang.json') as f:
    config = json.load(f)
...
{m}[{w}12{Fore.RESET}{m}]{Fore.RESET} NOTAS
    ...
    if choice == '8':
        Spinner()
        setTitle('Scrapper  |')
        tukan = input(f"[INPUT   ][{time.strftime('%H:%M:%S')}] Token: ")
        guildd = input(f"[INPUT  ][{time.strftime('%H:%M:%S')}] Server ID: ")
        chann = input(f"[INPUT   ][{time.strftime('%H:%M:%S')}] Channel ID: ")
    ...
    if choice == '10':
        Spinner()
        cantidad = int(input(f'[INPUT    ][{time.strftime("%H:%M:%S")}] Cuantas tokens genero?: '))
    ...
    if choice == '11':
        Spinner()
        setTitle(f"Acc Nuker    |")
        token = input(f'{y} [?] TOKEN: ')
        validateToken(token)
        Server_Name = str(input(f"[INPUT    ][{time.strftime('%H:%M:%S')}] Nombre del Servidor:  "))
        contenido_msg = str(input(f"[INPUT  ][{time.strftime('%H:%M:%S')}] Mensaje para MassDM: "))
        if threading.active_count() < threads:
            threading.Thread(target=utilidades.plugins.accsnuker.NOVASUN_START, args={token, Server_Name, contenido_msg}).start()
